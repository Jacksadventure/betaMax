...
def ensure_table_triple(conn: sqlite3.Connection):
    conn.execute(
        """CREATE TABLE IF NOT EXISTS mutations_triple (
...
def store_triple(
    ...
    fpath: str,
    pos_triple: Tuple[int, int, int],
    orig: str,
    mut: str
):
    conn.execute(
        "INSERT INTO mutations_triple (file_path, mutation_pos, original_text, mutated_text) "
        "VALUES (?, ?, ?, ?)",
        (fpath, f"{pos_triple[0]},{pos_triple[1]},{pos_triple[2]}", orig, mut),
    )
    conn.commit()
...
def process_file(
    ...
    cur = conn.execute("SELECT 1 FROM mutations_triple WHERE file_path=? LIMIT 1",
                       (str(path),))
    if cur.fetchone():
        return
